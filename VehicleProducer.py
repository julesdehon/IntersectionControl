...
class RandomVehicleProducer:
    def __init__(self, vehicle_list: List[Vehicle], rates: dict, algorithm: str, im: IntersectionManager,
    ...
    def step(self):
        ...
        self.vehicle_list[:] = [vehicle for vehicle in self.vehicle_list
                                if vehicle.get_id() not in arrived_vehicles]
        ...
        for road in self.spawn_probabilities.keys():
            ...
            if random.random() <= spawn_prob:
                ...
                self.vehicle_list.append(self.vehicle_for_algo(next_id))
    ...
    def vehicle_for_algo(self, vehicle_id) -> Vehicle:
        if self.algorithm == "qb-im":
            return QBIMVehicle(vehicle_id, self.im, self.environment, communication_range=75)
